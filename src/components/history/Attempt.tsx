...
export default function Attempt({
  ...
  let statsColor = "";
  const ratio = (attempt.score / attempt.total_questions) * 100;
  ...
  return (
    <div className="border p-[1rem] rounded-[0.5rem] my-[1rem]">
      <div className="flex justify-between">
        <h2 className="inline font-bold mb-[1rem] text-[1.2rem]">
          {attempt.roles.name}
        </h2>
        <span>{formatDateTime(new Date(attempt.completed_at))}</span>
      </div>
      <div
        className={`flex justify-between w-full leading-none text-[1rem]  ${className}`}
        {...props}
      >
        <div className="w-[6rem] flex items-center font-bold">
          <span
            className={`${statsColor} border rounded-full p-[0.5rem] me-[0.5rem]`}
          >
            {(attempt.score / attempt.total_questions) * 100}%
          </span>
          <span className={`${statsColor} border rounded-full p-[0.5rem]`}>
            {attempt.score + "/" + attempt.total_questions}
          </span>
        </div>
        <NavLink
          to={`/history/${attempt.id}`}
          className="h-full p-[0.5rem] text-center w-[5rem] text-[1rem] rounded-lg bg-gray-800 py-3 text-white font-semibold
