...
import { aggregate, getCorrect, getGrade } from '../../utils/results';

type Props = {
    className?: string,
    attempt: QuestionnaireResult
...
export default function Attempt({ className = '', attempt, ...props }: Props): JSX.Element {
    const stats = aggregate(attempt.userAnswers);
    const grade = getGrade(stats);
    const correct = getCorrect(stats);

    let statsColor = '';
    const ratio = stats.correct / stats.total;
    if (ratio >= .7) statsColor = 'bg-green-600 text-white'
    else if (ratio >= .5) statsColor = 'bg-yellow-600 text-white'
    else statsColor = 'bg-red-600 text-white'
    const [currentRole] = flashcards.filter((role) => role.id == attempt.roleId);
    return (
        <div className='border p-[1rem] rounded-[0.5rem] my-[1rem]'>
            <div className='flex justify-between'>
                <h2 className='inline font-bold mb-[1rem] text-[1.2rem]'>{currentRole.role}</h2>
                <span>{formatDateTime(new Date(attempt.submittedAt))}</span>
            </div>
            <div className={`flex justify-between w-full leading-none text-[1rem]  ${className}`} {...props}>
                <div className='w-[6rem] flex items-center font-bold'>
                    <span className={`${statsColor} border rounded-full p-[0.5rem] me-[0.5rem]`}>{grade}</span>
                    <span className={`${statsColor} border rounded-full p-[0.5rem]`}>{correct}</span>
                </div>
