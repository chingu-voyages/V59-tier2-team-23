...
export default function Questionnaire({
  ...
  const [generatedEssayQuestions, setGeneratedEssayQuestions] = useState<Essaycard[]>([])
  ...
  function RoleSelector({ roles, selectedRole, onSelect, onBegin }: RoleSelectorProps) {
    return (
  ...
  function QuestionSelector({ types, selectedType, onSelect, onBegin }: QuestionSelectorProps) {
    return (
      <div className='sm:min-h-screen bg-gray-50 flex items-center justify-center px-4 fluid-page-padding'>
        <div className='w-full max-w-md space-y-6 mt-10 mb-10'>
          <h2 className='text-2xl font-bold text-center'>Select Question Format</h2>
    ...
    return (
      <div className='min-h-screen bg-gray-50 flex items-center justify-center px-4 fluid-page-padding'>
        <div className='w-full max-w-md space-y-6 mt-10 mb-10'>
          ...
          <h3 className='text-lg font-semibold text-center p-3 rounded-lg'>{question.question}</h3>
  ...
  function Results({
    ...
    return (
      <div
        ...
        {...props}>
        <h1 className='text-[1.8rem] text-center  mb-[1rem]'>{selectedRole?.role} Quiz Results</h1>
        <div className='flex flex-col items-center justify-between w-full max-w-115 mb-[1rem] gap-[0.5rem]'>
          ...
          <div className='flex mt-4'>
            ...
            <button
              ...
              {isAddingQuestion ? "Creating Question..." : "Retry With More Questions"}
            </button>
          </div>
        </div>
        ...
        <ResultsGrid onReview={onReview} className='mb-[4rem]' result={result} />
  ...
  function Feedback({ question, userAnswer, onNext, onPrev, current, total }: FeedbackProps) {
    return (
      <div className='min-h-screen bg-gray-50 flex items-center justify-center px-4 fluid-page-padding'>
        <div className='w-full max-w-md space-y-6 mt-10 mb-10'>
          <div className='text-center text-sm text-gray-500'>
          ...
          <div className='space-y-3'>
            {Object.entries(question.options).map(([key, value]) => {
              ...
              return (
                <div key={key} className={`w-full rounded-lg border p-3 ${styles}`}>
                  <span className='font-semibold mr-2'>{key}.</span>
                  {value}
                </div>
              )
    ...
    return (
      <QuestionSelector
        ...
        onBegin={async () => {
          ...
          setCurrentIndex(0)
          setUserAnswers([])
          if (selectedType.type === "FREE_RESPONSE" || selectedType.type === "BOTH") {
            setIsGenerating(true)

            try {
              const aiQuestions = await handleGenerateFreeResponse(selectedRole.role)
          ...
          if (selectedType.type === "MULTIPLE_CHOICE") {
            const session = await startSession(selectedRole.id, 0, selectedRole.flashcards.length)
            if (session) {
    ...
    return (
      <Results
        onRetry={async (): Promise<void> => {
          ...
          const session = await startSession(selectedRoleInfo.id, 0, questions.length)
          if (session) {
            setSessionId(session.id)
          }
